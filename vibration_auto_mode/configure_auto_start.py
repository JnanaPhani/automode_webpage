...
Usage:
    python configure_auto_start.py <port> [baud_rate] [--output-type velocity|displacement]
    python configure_auto_start.py <port> --displacement
    python configure_auto_start.py --list-ports
    python configure_auto_start.py --help
    
Examples:
    # Linux - Velocity output (default)
    python configure_auto_start.py /dev/ttyUSB0
    python configure_auto_start.py /dev/ttyUSB0 460800
    
    # Displacement output
    python configure_auto_start.py /dev/ttyUSB0 --displacement
    python configure_auto_start.py /dev/ttyUSB0 --output-type displacement
    
    # Windows
    python configure_auto_start.py COM3
    python configure_auto_start.py COM3 --displacement
    
    # macOS
    python configure_auto_start.py /dev/tty.usbserial-1410
    python configure_auto_start.py /dev/tty.usbserial-1410 --displacement
...
def configure_sensor(port: str, baud: int, output_type: str = "velocity") -> bool:
    ...
    try:
        ...
        if output_type.lower() not in ["velocity", "displacement"]:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Linux - Configure for velocity output (default)
  python configure_auto_start.py /dev/ttyUSB0
  python configure_auto_start.py /dev/ttyUSB0 460800
  
  # Configure for displacement output
  python configure_auto_start.py /dev/ttyUSB0 --displacement
  python configure_auto_start.py /dev/ttyUSB0 --output-type displacement
  ...
  python configure_auto_start.py --list-ports
    ...
    parser.add_argument(
        ...
        default="velocity",
        help="Set output type: 'velocity' (default) or 'displacement'",
    )
    parser.add_argument(
        "--displacement",
        action="store_const",
        const="displacement",
        dest="output_type",
        help="Shorthand for --output-type displacement",
    )
    ...
    # Configure sensor
    success = configure_sensor(args.port, args.baud, output_type=args.output_type)
